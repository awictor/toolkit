...
check('exactly 136 tool cards', () => {
  assert.equal((html.match(/class="card"/g) || []).length, 136);
});
...
check('valid JSON-LD ItemList with 136 items', () => {
  const m = html.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/);
  assert.ok(m, 'no JSON-LD block');
  const data = JSON.parse(m[1]);
  assert.equal(data['@type'], 'ItemList');
  assert.equal(data.itemListElement.length, 136);
...
check('sitemap.xml lists all 136 pages', () => {
  ...
  assert.equal((sm.match(/<loc>/g) || []).length, 137);
