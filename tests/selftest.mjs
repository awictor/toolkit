...
check('exactly 137 tool cards', () => {
  assert.equal((html.match(/class="card"/g) || []).length, 137);
});
...
check('valid JSON-LD ItemList with 137 items', () => {
  const m = html.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/);
  assert.ok(m, 'no JSON-LD block');
  const data = JSON.parse(m[1]);
  assert.equal(data['@type'], 'ItemList');
  assert.equal(data.itemListElement.length, 137);
...
check('sitemap.xml lists all 137 pages', () => {
  ...
  assert.equal((sm.match(/<loc>/g) || []).length, 138);
